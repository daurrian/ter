...
\begin{document}
  ...
  \begin{lean4code}
    class IsGibbs (f : Bernoulli Z n → R) (mu : Measure (Bernoulli Z n))
    extends InvariantProb mu where
      gibbs_prop : Exists P : R, Exists c c' : NNReal, c > 0 /\ c' > 0 /\ 
      Forall x : Bernoulli Z n, Forall m : N,
    mu (cylinder x m) / nnexp (- P * m + ∑ k ∈ Ico 0 m, f (shift^[k] x))
      ∈ Icc (c : ENNReal) (c': ENNReal)
  \end{lean4code}


  \begin{lean4code}
    theorem gibbs (f : Bernoulli Z n → R) [HolderLike f] :
      ExistsUnique mu : Measure (Bernoulli Z n), IsGibbs f mu := by sorry
  \end{lean4code}
  ...
  \begin{theorem*}
    Soit $\O$ un ouvert borné de $E$. Alors lafonction $\Phi$ vérifie les propriétés suivantes :
    \begin{enumerate}
